    ...
    return Promise.all([datasetsFor(dashboardId), fetchOnce('dashboards.json')])
      .then(function (both) {
        ...
        return Promise.all(list.map(function (d) {
          ...
          // Every period a dataset actually has, not just the newest one
          // -- a register with July and August both loads July and August.
          // Each dataset's own periods, independently: a row already
          // carries its own real date, so cross-register comparisons (a
          // local-purchase GRN against the PO it names, a return against
          // the receipt it came from) already match up per-row from that,
          // not from both files having been the one upload -- there is no
          // "different month" state left to guard against here. This was
          // once restricted to a single shared month for exactly that
          // guard; see git history if that reasoning is ever needed again.
          //
          // Not yet bounded by how much history has piled up -- fine while
          // a register has a handful of months on record; worth capping to
          // a trailing window (or a picker) once one has a year or more.
          var partial = [];
            ...
            months.forEach(function (period) {
              ...
              if (parts !== true) {
                partial.push({ label: x.label, dataset: x.dataset,
                               period: period, missingParts: parts });
              }
            });
          });
